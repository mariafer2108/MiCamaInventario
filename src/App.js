...
const MobileInventoryCard = ({ item, setSelectedInventoryDetails, setIsInventoryDetailsModalOpen, sellItem, editItem, deleteItem, isGuest }) => {
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
      ...
      <div className="flex justify-between items-center pt-3 border-t border-gray-100">
        ...
        <button
          ...
          disabled={isGuest || item.cantidadstock === 0}
...
const MobileSalesCard = ({ sale, openSaleDetails, openEditSale, deleteSale, isGuest }) => {
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
      <div className="flex justify-between items-start mb-3">
        <div>
...
const checkUser = async () => {
  try {
    ...
    if (currentUser) {
      ...
      if (guestRaw) {
        setUser(createGuestUser(currentUser));
      } else {
        setUser(currentUser);
      }
      return;
    ...
    try {
      const guestRaw = localStorage.getItem(GUEST_STORAGE_KEY);
      if (guestRaw) setUser(createGuestUser());
    } catch (_) {
      setUser(null);
    }
  }
...
const loadData = async () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (user?.isGuest) {
      setMessage({ type: 'error', text: 'El modo invitado es solo lectura.' });
      return;
    }
        ...
        <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <button
              ...
              className="w-full bg-gray-100 text-gray-800 py-2 px-4 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-400"
            >
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className={`bg-blue-600 text-white px-4 py-2 rounded-md flex items-center text-sm ${user?.isGuest ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`}
                >
                ...
                <div key={item.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-gray-100">
                    ...
                    <button
                      ...
                      disabled={user?.isGuest || item.cantidadstock === 0}
                      title="Vender producto"
                    >
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
              ...
              <button
                ...
                disabled={user?.isGuest}
                className={`px-4 py-2 bg-blue-600 text-white rounded-lg flex items-center gap-2 ${user?.isGuest ? 'opacity-50 cursor-not-allowed' : 'hover:bg-blue-700'}`}
              >
                <Edit2 className="w-4 h-4" />
                Editar
              </button>
              <button
                ...
                disabled={user?.isGuest}
                className={`px-4 py-2 bg-red-600 text-white rounded-lg flex items-center gap-2 ${user?.isGuest ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-700'}`}
              >
                <Trash2 className="w-4 h-4" />
                Eliminar
              </button>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3 mt-6 pt-4 border-t">
              ...
              <button
                ...
                className={`px-4 py-2 bg-green-600 text-white rounded-lg flex items-center gap-2 ${(user?.isGuest || selectedInventoryDetails.cantidadstock === 0) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-700'}`}
                disabled={user?.isGuest || selectedInventoryDetails.cantidadstock === 0}
              >
                <DollarSign className="w-4 h-4" />
                Vender
              </button>
